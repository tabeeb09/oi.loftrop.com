...
require_root() {
  if [[ "${EUID:-$(id -u)}" -ne 0 ]]; then
    echo "Run as root, for example: sudo bash scripts/setup-app-vps.sh" >&2
    exit 1
  fi
}
...
configure_base_env() {
  ...
  echo "Configuring non-secret app deployment values in $BASE_ENV_FILE"
  ...
  prompt_if_placeholder BAO_ADDR "OpenBao URL reachable from this app VPS" "https://bao.example.com"
...
main() {
  ...
  PROJECT_ROOT="$PROJECT_ROOT" PROJECT_NAME="$PROJECT_NAME" bash ./scripts/deploy-app-vps.sh
}
