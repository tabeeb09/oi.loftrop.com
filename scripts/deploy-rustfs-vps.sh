...
docker compose -p "$RUSTFS_COMPOSE_PROJECT" -f docker-compose.rustfs.yaml --env-file "$DEPLOY_ENV_FILE" pull
docker compose -p "$RUSTFS_COMPOSE_PROJECT" -f docker-compose.rustfs.yaml --env-file "$DEPLOY_ENV_FILE" up -d
