# Hetzner Split-VPS Bootstrap

This layout provisions separate Hetzner Cloud VPSes for CAId/OpenBao/Keycloak, RustFS storage, and the website application. Use it when storage should be shared across projects or when you want cleaner isolation than the single-VPS layout.
...
```powershell
.\scripts\bootstrap-hetzner-project.ps1 --config infra\hetzner\bootstrap.config.example.json
```
...
- `dnsProvider`: set to `cloudflare` to let the bootstrapper create/update the DNS records after Terraform returns the VPS IPs.
